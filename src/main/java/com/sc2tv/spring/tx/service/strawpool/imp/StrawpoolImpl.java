...
import com.sc2tv.spring.tx.service.Scanner;
...
public class StrawpoolImpl implements Strawpool {
    ...
    public void startVoteStrawpool(String url, final int[] voteFor, int threads) {
        ...
        List<ProxyUnit> proxyUnits;
        if((proxyUnits = proxyManager.getAvaliableProxies()).size()<200)
            proxyUnits = proxyManager.getProxyUnitList();
        if (voteFor[0] != -1) {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            if (outPut.contains("image") && money) {
                                ...
                                pairs.add(new BasicNameValuePair("code", new CaptchaRecognition().solveCaptchaMulty(image)));
                               ...
                               // params.put("SubmitButton", "%CF%EE%E4%F2%E2%E5%F0%E4%E8%F2%FC+%CA%EE%E4");
                                //params.put("code", new CaptchaRecognition().solveCaptchaMulty(image);
                                count[0]++;
                                webClient1.executePost("http://www.rupoll.com/voteconfirm.php", pairs);
                            }
                ...
                executor.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            String outPut = "";
                            try {
                                outPut = webClient1.executeGet("http://www.rupoll.com/voteconfirm.php", params);
                            } catch (Exception exp) {
                                return;
                            }
            ...
            public void run() {
                while (true) {
                    ...
                    if (STR) {
                        ...
                        if (!task.contains(strawpool)) {
                            task.add(strawpool);
                            System.out.println(String.format("Starting vote STR in chat: %s - %s - STR: %s", channels.getStreamerNameByChannelId(scanner.getChatId()), "http://sc2tv.ru/content/" + channels.getTitleByChannelId(scanner.getChatId()), strawpool));
                    ...
                    if (RUP) {
                        ...
                        if (!task.contains(rupool)) {
                            task.add(rupool);
                            System.out.println(String.format("Starting vote STR in chat: %s - %s - STR: %s", channels.getStreamerNameByChannelId(scanner.getChatId()), "http://sc2tv.ru/content/" + channels.getTitleByChannelId(scanner.getChatId()), rupool));
